...
def _default_cfg():
    return {
        ...
        "reward_scales": {
            ...
            "lin_vel_z":         -2.0,
            "ang_vel_xy":        -0.05,
            "orientation":       -1.0,     # 2026-08-15 (T079s): -5.0 -> -1.0 (G1 と同値)。
                                           # 質量修正後、残る失敗は「潰れ」ではなく「転倒」だと
                                           # 実測で判明した(256env/15秒/無擾乱):
                                           #   時間切れ(完走) 78.1% / term_tilt 16.0% / term_low 5.9%
                                           # 学習ログの fall_by_low 0.56 は「転倒したものの内訳」
                                           # かつ外乱push有りの値で、全体比率ではなかった。
                                           # 残る term_low も 7.1段目で直前0.5秒に0.398m落ちる
                                           # 本物の潰れで、判定の誤発火ではない。
                                           # orientation が G1 の5倍厳しいと体幹を垂直に固めすぎ、
                                           # バランス回復に必要な傾きまで罰する懸念がある
                                           # (人間も階段で崩れた際は体幹を傾けて回復する)。
                                           # 効果が無ければ -5.0 へ戻す。
            "base_height":      -30.0,
            "action_rate":       -0.01,    # C5
            "dof_vel":           -1e-4,
