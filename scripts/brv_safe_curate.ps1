...
function Write-GitMirrorMemory {
    param(
        [string]$Reason,
        [string]$ContextText,
        [string[]]$SourceFiles,
...
try {
    ...
    if (-not $curateCompleted -and ($process.ExitCode -ne 0 -or $curateErrored)) {
    ...
    Write-GitMirrorMemory -Reason "brv curate completed successfully" -ContextText $Context -SourceFiles $Files -OutputPath $MirrorPath -CurateStatus "completed"
    exit 0
} finally {
