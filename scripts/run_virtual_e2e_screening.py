...
def main() -> int:
    ...
    void_run = subprocess.run(["python", "scripts/derive_void_mechanisms.py", "--source", str((args.quality_out / f'{args.case}_quality_screening.vtu').resolve()), "--output", str(void_out.resolve()), "--card", str(args.card.resolve())], capture_output=True, text=True)
