...
## 15. GitHub バックアップ & CI 整合性プロトコル (GitHub Backup & CI Integrity)

Antigravity は、GitHub へのバックアップ（プッシュ）を行った際、以下のプロセスを完遂する義務を負う。

### 15-1. プッシュ後の整合性確認 (Verification)
- コードをプッシュした後、可能な限り GitHub Actions などの CI ステータスを確認する。
- ユーザーからエラー報告があった場合、あるいは自らエラーを検知した場合は、即座に調査を開始する。

### 15-2. 自己修復 (Self-healing & 3-Retry Protocol)
- CI エラーの根本原因を特定し、コードまたは設定の修正を行う。
- **3回までの自律試行**: 1回目の修正で CI がパスしない場合、最大 3 回まで「再調査・別アプローチでの対策・再プッシュ」を自律的に繰り返す。
- **段階的深掘り**: 修正が失敗するたびに、その結果（エラーログの変化等）を新たなエビデンスとして原因究明をやり直し、より多角的な対策を盛り込む。

### 15-3. 知見の永続化 (Documentation & Growth)
- **すべての試行を記録**: 3回目までの各試行における「原因究明結果」「実施した対策内容」「結果」を、漏らさず以下の場所に記録する。
    - **インシデントログ**: `docs/INCIDENT_LOG.md` に、成功・失敗に関わらずすべてのステップ（試行回数ごとの分析と対策）を追記する。
    - **ByteRover**: `brv curate` を使用し、特に「なぜ1回目で直らなかったか」の反省を含めたコンテキストを保存する。
- **RL 自己成長**: 失敗パターンを `rl_growth` スキルを通じて Qdrant に保存し、将来の同様のミスを防止する。

---
*Last Updated: 2026-05-05 10:59 (JST)*
...
## Python Windows Encoding Standard (MANDATORY — P023 / INC-082)

All Python files generated or modified by any AI agent MUST include the following at module top, before any I/O or DB library imports:
...
For any module that connects to PostgreSQL, also add:
...
Prohibited in `print()` statements: U+2014 (`--`), U+2192 (`->`), emoji. Use ASCII equivalents `--`, `->`, `[OK]`, `[NG]`.

Reason: Japanese Windows sets `sys.stdout.encoding=cp932`. A non-cp932 character inside a `try/except Exception` block will be silently caught and misidentified as a DB or network error (INC-082, RPN=504).

Full rule: `GOVERNANCE.md` Section 3 / `data/workspace/PROMISES.md` P023
