...
original_obj.name = "Bulma_Mesh"

# 1. メッシュを複製して Voxel Remesh（穴を埋める）
bpy.ops.object.select_all(action='DESELECT')
original_obj.select_set(True)
bpy.context.view_layer.objects.active = original_obj
bpy.ops.object.duplicate()
voxel_obj = bpy.context.active_object
voxel_obj.name = "Voxel_Helper"

# Remesh Modifier 適用 (Voxel モード)
mod = voxel_obj.modifiers.new(name="Remesh", type='REMESH')
mod.mode = 'VOXEL'
mod.voxel_size = 0.02 # 精度
...
# 3. Voxelメッシュに自動ウェイトを適用（これは高確率で成功する）
bpy.ops.object.select_all(action='DESELECT')
...
# 4. ウェイトを元のメッシュに転送 (Data Transfer)
bpy.ops.object.select_all(action='DESELECT')
original_obj.select_set(True)
bpy.context.view_layer.objects.active = original_obj
...
print('Voxel-based professional rigging completed')
