...
- 2026-09-19 15:46 JST追記: 新規launcherは既存`preflight_started.json`または`preflight_result.json`のあるcase pathを拒否し、古い解析証拠を上書きしない。新しいCaseDirを要求する。生成BashのGit Bash実行テストでclean directoryは許可、両marker存在時は拒否・保持を確認。対象4テスト群31件PASS。変更前snapshotは`backups/openfoam-case-generation-guard-20260919`。WSL solver・実機は未起動。
