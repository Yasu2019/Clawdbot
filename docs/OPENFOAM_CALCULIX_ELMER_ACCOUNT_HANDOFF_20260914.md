...
- `scripts/start_openfoam14_wsl_detached.ps1`を改訂: Global named mutexで同一端末からの別launcher同時起動を直列化し、プリフライト〜dispatch受理後に必ず解放する。Task Scheduler worker、WSL readiness marker、readiness確認後のみsolver dispatch、20/25/30秒bounded call、dispatch exit/output記録、状態不明時fail-closedも実装。指定unitだけでなくWSL内の`foamRun`/`mpirun`全体を読み取り専用で監査し、既存solverが一つでもあればtask登録/solver dispatchを拒否する。削除時はランダムworker設定fingerprint一致を確認し、既存unit/タスクを自動停止・上書きしない。worker引数は検証済みscalarのみで、shellは固定templateから再構築する（caller提供Base64 shellを実行しない）。全引数をWSL照会前に検証し、`-ValidateOnly`で外部副作用なしに検査できる。worker終了時はmonitor状態・exit/output、runner `stop_reason`、solver exit code、checkpoint妥当性、latest time、fatal countをhost manifestへ追記する。
- 検証: PowerShell AST parse PASS、対象テスト27 PASS。埋込みBashの`bash -n`、strict JSON/stop_reason確認（途中書込みを考慮した最大5回retry）、Task Scheduler削除helper、active solver audit mockに加え、WSL/Task Schedulerをmockしたworker統合試験でhost manifest更新・cleanupまで検証（実WSL/タスクなし）。`-ValidateOnly`正常経路とNaN終了時刻拒否も実Windows PowerShellで確認。直近変更前snapshot: branch `backups/openfoam-launcher-post-input-20260919`, commit `74e1e4fb7e`; 初期snapshot: branch `backups/openfoam-launcher-prechange-20260919`, commit `8060cf0d35`; archive SHA256 `07EF620386BF14367955E4D2A59AAF4A43097FB21B6822485731E69217596C84`。
- 2026-09-19 15:32 JST追記: launcherのhost manifest書込みを同一ディレクトリ内の一時ファイル経由に変更し、既存JSONは`File.Replace`で原子的に差し替える。初回作成・既存manifest更新の両方をWindows PowerShell実動テストし、JSON再読込および`.tmp`/`.bak`残留なしを確認。関連テストは30件。変更前snapshotは`backups/openfoam-manifest-atomic-20260919`。これはランチャーのローカル検証であり、Red LAVIE実機配置・OpenFOAM計算の完走を意味しない。
- 2026-09-19 15:35 JST追記: manifest置換中に対象JSONが排他ロックされている失敗経路をPowerShellで実行し、書込み失敗時も旧JSONが保持され、一時/backupファイルが残らないことを確認した。対象4ファイルのテスト30件PASS。変更前snapshotは`backups/openfoam-manifest-failure-atomic-20260919`。
- 2026-09-19 15:39 JST追記: keepalive workerがmanifestを更新する前にschema、Task Scheduler task名、systemd unit、case pathがworker設定と一致することを検証。一致しないmanifestでは更新拒否し、別アプリのJSONを変更しない統合テストPASS。全関連テスト30件、PowerShell AST、`-ValidateOnly` PASS。変更前snapshotは`backups/openfoam-manifest-owner-guard-20260919`。Red LAVIE実機配置・solver実行は未実施。
- 2026-09-19 15:46 JST追記: 新規launcherは既存`preflight_started.json`または`preflight_result.json`のあるcase pathを拒否し、古い解析証拠を上書きしない。新しいCaseDirを要求する。生成BashのGit Bash実行テストでclean directoryは許可、両marker存在時は拒否・保持を確認。call-siteがTask Scheduler/solver dispatchより前であることも検査。対象4テスト群32件PASS。変更前snapshotは`backups/openfoam-case-generation-guard-20260919`。WSL solver・実機は未起動。
- 2026-09-19 15:55 JST追記: workerのterminal manifest判定を強化。runnerのschemaに加えて`case_dir`一致、有限なlatest/end time、completed時のexit code 0とlatest time >= requested end time（丸め許容）を必須とし、他caseの古い結果や不整合な完了記録を成功扱いしない。異常fixture 4種を含む関連32 tests PASS、PowerShell ASTとside-effect-free `-ValidateOnly` PASS。snapshotは`backups/openfoam-terminal-manifest-integrity-20260919`。solver未起動。
- 2026-09-19 15:59 JST追記: `systemd-run`の30秒呼出しがtimeoutしたとき、未確認のままreject扱いしない。host manifestは`dispatch_outcome_unknown`とエラーを記録し、keepaliveを最大90秒unit開始待ちに延長。workerはこの不確定状態を実unitのterminal resultで解決する。mock統合でtimeout相当の状態がcompletionへ解決されることを検証。対象4テスト群32件PASS。snapshotは`backups/openfoam-dispatch-timeout-state-20260919`。
- 2026-09-19 16:04 JST追記: 既定の固定manifest名による前回記録の上書きを防ぐ。既定出力はunit名＋GUID付きのrun固有JSONへ変更し、明示`-ManifestPath`が既存の場合はWSL照会前に拒否。初回書込みも`File.Move`のno-overwrite作成にし、競合で既存ファイルを上書きしない。PowerShellで明示既存ファイル保全・create-only/atomic replace失敗を検証。関連33 tests PASS、AST/ValidateOnly PASS。snapshotは`backups/openfoam-manifest-generation-unique-20260919`。
- 2026-09-19 16:07 JST追記: `stop_reason=completed`だけで成功にしない契約を追加。`checkpoint_valid=true`、必須field全件（`alpha.polymer,T,p_rgh,U,rho`）、fatal_count=0、exit=0、case一致、finite時刻、latest >= requested_endが揃わない場合はterminal result invalidとする。失敗fixtureを追加し、対象33件PASS。snapshotは`backups/openfoam-checkpoint-success-contract-20260919`。
...
- 2026-09-19 16:13 JST追記: launcher親プロセスとkeepalive workerが同じhost manifestを同時更新して、互いの項目を失う競合を防ぐため、worker側のread/identity-check/modify/atomic-write全体をlauncherと同じGlobal named mutexで直列化。タイムアウト時はmanifestを変更せず失敗し、mutex abandoned時は所有を引き継いでfinallyで解放する。順序と共通mutex名を検査するテストを追加。関連34件PASS、PowerShell ASTとside-effect-free `-ValidateOnly` PASS。変更前snapshot `backups/openfoam-worker-manifest-serialization-20260919`。検証はローカルのみで、WSL/solverは起動していない。
