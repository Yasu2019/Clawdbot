...
- **Failure signatures, in the order they were peeled back:**
  ...
  6. Found only after the manual reproduction passed and the continuous track was watched: `tri_track_param_overrides.json` pinned `inlet_velocity` to 1.0 m/s, applied by `meaning_gate_auto_improver` on 2026-07-19. The orchestrator's `_apply_llm_overrides` writes it over whatever the sampler proposes, so every automatic trial ran the gate ten times slower than the proven 6.51 m/s and reached 2-6% fill. The manual dispatch bypassed the orchestrator and therefore never saw it.
...
- **Prevention rules:**
  ...
  - IF a manual dispatch passes but the continuous track keeps failing, THEN diff the effective parameters, not the code, BECAUSE `_apply_llm_overrides` applies `tri_track_param_overrides.json` on the orchestrator path only, and an auto-applied override can survive long after the geometry it was tuned for is gone.
  - IF an automatic improver pins a physical parameter, THEN record the geometry it was tuned for and re-validate it whenever the geometry changes, BECAUSE a gate velocity that suited the plate cannot fill a 41.33 cm3 shell within the Moldflow-derived horizon.
...
## INC-162: ThinkPad DXF2STEP and FEM Impact were not operating as a guarded 1:1 pair

- **Date / detection:** 2026-07-25 JST; the user requested continuous concurrent use of both ThinkPad workloads.
- **Impact:** The FEM Impact tri-track thread was commented out, DXF2STEP slept 600 seconds between cycles and retained a stopped meaning gate, while the remote ThinkPad had five orphaned FreeCAD workers and two FEM Impact processes.
- **Root cause (5 Why):** (1) FEM Impact did not start from the continuous orchestrator. (2) Its thread had been disabled and its bench policy required an idle ThinkPad. (3) DXF2STEP and FEM Impact were independently restarted without a shared one-slot cleanup contract. (4) controller timeouts/restarts left remote solver children alive. (5) the previous policy optimized fail-closed safety and idle dispatch rather than the newly requested continuous 1:1 utilization.
- **Correction:** Re-enabled the FEM Impact thread; disabled its idle-only requirement; set both loop intervals to 15 seconds; assigned FEM Impact to CPU 0-5 with nice level 5; set shared launch limits to CPU 85%, RAM 85%, and 80 C; disabled consecutive-quality-failure shutdown for the two ThinkPad tracks; added type-scoped orphan cleanup before each new launch; cleared the stale DXF2STEP stop flag and restarted both watchdogs.
- **Files:** `scripts/k10_tri_track_cae_orchestrator.py`, `scripts/k10_thinkpad_dxf2step_loop.py`, `data/workspace/cae_workload_router.yaml`, `data/workspace/thinkpad_dxf2step_te_config.json`, `scripts/start_k10_tri_track_cae_watchdog.ps1`, `scripts/start_k10_thinkpad_dxf2step_watchdog.ps1`.
- **Verification:** Python compilation, JSON/YAML assertions, mocked three-track construction, and FEM dry-run passed. Live ThinkPad verification showed exactly one `run.Impact` process, nice=5, affinity=0-5; DXF2STEP completed job `tp-dxf-1ac1c3a5`, recorded `poll_seconds=15`, `running=true`, and removed the stale meaning gate. Measured load during verification was CPU 14.9%, RAM 44.2%, temperature 71 C.
- **Prevention:** Each workload owns one remote slot. A new job is admitted whenever the load guard is below threshold; excessive load pauses only new dispatch and the 15-second loop resumes automatically after recovery.
- **Rollback:** Set FEM `only_when_thinkpad_idle: true`, restore the former poll intervals/thresholds, and disable the `fem_impact_thinkpad` thread.
- **Web knowledge:** Not used; live K10/ThinkPad process, status, affinity, and load evidence were authoritative.

---

## INC-163: DXF2STEP Telegram reports could send duplicate or near-duplicate images

- **Date / detection:** 2026-07-25 JST; user reported repeated Telegram messages and images after job `tp-dxf-3a9902e0`.
- **Impact:** A single successful trial could produce redundant Telegram traffic; replaying the same Job ID could resend the complete report.
- **Root cause:** The reporter intentionally collected both `combined_views.png` and layer `*_views.png`, even for a one-layer solid. When original-DXF rendering succeeded but comparison-panel composition failed, the fallback branch sent the primary 3D image and a later branch sent it again. No persistent Job-ID receipt or concurrent-delivery lock existed.
- **Correction:** Unified the fallback send path; suppress layer views for one-layer jobs; remove exact SHA-256 duplicate previews; add archive-local `telegram_delivery_receipt.json` and `.telegram_delivery.lock`; skip a completed Job ID on replay.
- **Files:** `scripts/dxf2step_telegram_report.py`, `data/workspace/tests/test_dxf2step_telegram_dedup.py`.
...
- Cause: the legacy script has no argument parser and ignores `--help`.
- Containment: stopped only the exact launcher processes; allowed the in-scope
...
- Validation: PowerShell AST parse PASS; 34 focused launcher/preflight/checkpoint tests PASS. Runtime tests cover process filtering, mocked worker finalization/cleanup, cross-process global mutex exclusion, atomic host-manifest create/replace, locked-target replacement failure preserving the prior JSON with no temp/backup residue, fresh-case marker rejection before task registration/solver dispatch, strict terminal case/time/completion/checkpoint-field/fatal-count consistency, ambiguous dispatch resolution, refusal to overwrite an existing launch manifest, and worker serialization of manifest read/modify/write under the launcher mutex. Default manifests use unique per-run names; explicit existing paths fail before WSL. Worker config is validated scalar data only and reconstructs its fixed template; it records runner stop reason, exit/checkpoint/latest-time/fatal facts. Side-effect-free `-ValidateOnly` passes. Pre-change snapshots pushed on `backups/openfoam-worker-manifest-serialization-20260919`, `backups/openfoam-checkpoint-success-contract-20260919`, `backups/openfoam-manifest-generation-unique-20260919`, `backups/openfoam-dispatch-timeout-state-20260919`, `backups/openfoam-terminal-manifest-integrity-20260919`, `backups/openfoam-case-generation-guard-20260919`, `backups/openfoam-manifest-owner-guard-20260919`, `backups/openfoam-manifest-failure-atomic-20260919`, and `backups/openfoam-manifest-atomic-20260919`; prior backups: `backups/openfoam-launch-singleton-20260919`, `backups/openfoam-host-result-facts-20260919`, `backups/openfoam-keepalive-worker-hardening-20260919`, `backups/openfoam-launcher-post-input-20260919` at `74e1e4fb7e`, and initial archive SHA256 `07EF620386BF14367955E4D2A59AAF4A43097FB21B6822485731E69217596C84`.
- Additional safety gates: the worker rejects host manifests whose schema/task/unit/case identity does not match; the launcher refuses to reuse a case directory containing a prior `preflight_started.json` or `preflight_result.json`, preserving the old run evidence.
...
# INC-196 -- CAE verification postprocessor false-HOLD / false-candidate risks (2026-09-20)

- Trigger 1: Elmer 8.4 returned code 0 and finished the thermal/stress solve, but the first verifier reported HOLD because it searched only the case root for `case.result` and VTU. With `Mesh DB "." "mesh"`, the files were under `mesh/`.
- Trigger 2: review of the first weld-front collision detector found that predecessor lag `=0` was accepted. An initially filled gate blob on an unstructured mesh could therefore be mislabeled as a weld collision.
- Impact: test-generation postprocessors only. No running solver, historical result, or user-owned artifact was stopped or overwritten. The Elmer solve was real but initially classified incorrectly; the weld risk was caught before publication.
- 5 Why / logical tree: output absent at expected path -> verifier assumed one output directory -> SIF mesh subdirectory semantics were not represented -> only root-level happy-path was considered -> recursive artifact discovery regression was missing. Initial blob false candidate -> equal-time neighbours accepted -> predecessor relation used `>=` instead of strict `>` in time -> initialization and transported front were not separated -> simultaneous-arrival negative control was missing.
- FMEA: false HOLD severity 4, occurrence 4, detection 3 (RPN 48); false weld candidate severity 7, occurrence 4, detection 4 (RPN 112). Controls are recursive result discovery with relative path/SHA, strict positive predecessor lag, and dedicated regressions.
- Fix: recursively discover exactly one `case.result` and a parseable VTU, persist their relative paths and SHA256; require a predecessor to arrive strictly before its collision cell; add deformed-VTU-coordinate and simultaneous-initial-blob tests.
- Verification: real Docker Elmer 8.4 solve returns `NUMERICALLY_COMPLETE_UNCALIBRATED`, analytical displacement relative L2 `0`, and six tests pass. Weldline tests include one-way chain 0 candidates, opposing merge 180 degrees, persistent union, and simultaneous initial blob 0 candidates; five tests pass.
- Web decision: no new web search was required. The actual SIF, emitted directory tree, solver log, VTU content, and deterministic synthetic fields fully identified both local implementation defects; external knowledge would not change the countermeasure.
- Recovery: use commits before the affected additions (`61b1a6c488` for the prior verified checkpoint), preserve all solver outputs, and revert only the two new postprocessor files if a regression is found.
- Obsidian: `data/state/Obsidian Vault/60_PC_Logs/CAE_INC-196_20260920.md`.
