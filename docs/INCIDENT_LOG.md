...
- Validation: PowerShell AST parse PASS; 29 focused launcher/preflight/checkpoint tests PASS. Runtime tests cover process filtering, mocked worker manifest finalization/cleanup, and cross-process global named mutex exclusion. Worker config is validated scalar data only and reconstructs its fixed template; it records runner stop reason, exit code, checkpoint validity, latest time, and fatal count. The launcher validates inputs before WSL and has side-effect-free `-ValidateOnly`. Pre-change snapshot pushed on `backups/openfoam-launch-singleton-20260919`; prior backups: `backups/openfoam-host-result-facts-20260919`, `backups/openfoam-keepalive-worker-hardening-20260919`, `backups/openfoam-launcher-post-input-20260919` at `74e1e4fb7e`, and initial archive SHA256 `07EF620386BF14367955E4D2A59AAF4A43097FB21B6822485731E69217596C84`.
