...
- Validation: PowerShell AST parse PASS; 30 focused launcher/preflight/checkpoint tests PASS. Runtime tests cover process filtering, mocked worker finalization/cleanup, cross-process global mutex exclusion, atomic host-manifest create/replace, and locked-target replacement failure preserving the prior JSON with no temp/backup residue. Worker config is validated scalar data only and reconstructs its fixed template; it records runner stop reason, exit/checkpoint/latest-time/fatal facts. Side-effect-free `-ValidateOnly` passes. Pre-change snapshots pushed on `backups/openfoam-manifest-failure-atomic-20260919` and `backups/openfoam-manifest-atomic-20260919`; prior backups: `backups/openfoam-launch-singleton-20260919`, `backups/openfoam-host-result-facts-20260919`, `backups/openfoam-keepalive-worker-hardening-20260919`, `backups/openfoam-launcher-post-input-20260919` at `74e1e4fb7e`, and initial archive SHA256 `07EF620386BF14367955E4D2A59AAF4A43097FB21B6822485731E69217596C84`.
- Additional ownership gate: before updating an existing host manifest, the worker verifies the schema, task name, unit, and case path against its own validated config. A mocked worker integration rejects foreign JSON and preserves it unchanged. Pre-change snapshot pushed on `backups/openfoam-manifest-owner-guard-20260919`.
