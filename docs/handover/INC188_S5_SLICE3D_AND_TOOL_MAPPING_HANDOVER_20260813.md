...
根本原因: パンチの細かい輪郭（丸穴φ0.56mm・角R0.22）がグリッド解像度でエイリアシングし、
接触瞬間の5m/s速度不連続が1〜2セルへ集中する。工具すきま（FEM確立値5〜40µm）を解像する
には dx~5〜10µm が必要（現行比セル数**数千倍**）で、計算コストが非現実的。BC緩和・速度
ランプは接触の立ち上がりを遅延できるが、根本の解像度不足は解消しない。

**⚠ 同種の問題を一様グリッドMPMで評価する前に**: グリッド解像度と最小工具形状特徴
（角R・穴径）の比を先に見積もること。目安 R/dx ≥ 10 を満たさなければ着手しない。
満たせない場合は FEM（適応メッシュ）を使うか、MPM なら適応/疎グリッド(octree等)の
実装が前提になる（本件はそこまでは実装していない）。
