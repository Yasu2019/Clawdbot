...
create_hist(sim_x, "hist_x_v6.png", "X-Axis Gap", stats_x)
create_hist(sim_y, "hist_y_v6.png", "Y-Axis Gap", stats_y)
...
img_x = get_b64("hist_x_v6.png")
img_y = get_b64("hist_y_v6.png")
...
if os.path.exists(THREE_JS_PATH):
    ...
    print("WARNING: three.min.js not found. 3D viewer will fail.")
...
<head>
    ...
    <title>Gap Analysis Report v6 (All-in-One)</title>
    <style>
...
<div class="container">
    <h1>Tolerance Gap Analysis Report (v6)</h1>
    <p>Single-File Mode (No External Dependencies)</p>
    
    <div class="viewer-box" id="viewer3d">
        <div id="loading">Initializing...<br><span style="font-size:0.8em; color:#aaa;">Loading Embedded Engine</span></div>
...
<script>
    ...
    window.onerror = function(msg, url, line) {{
        ...
        el.innerHTML += `<strong>CRITICAL ERROR:</strong> ${{msg}}<br>Line: ${{line}}<hr>`;
    ...
    function main() {{
        ...
        if (typeof THREE === 'undefined') {{
            throw new Error("Three.js library failed to load! The embedded script might be truncated or invalid.");
        }}
        ...
        function addPoints(vertices, color, size) {{
            ...
            # Using simple material to avoid shader compilation errors on some GPUs
            const mat = new THREE.PointsMaterial({{ color: color, size: size }});
            const mesh = new THREE.Points(geo, mat);
            scene.add(mesh);
        }}

        addPoints(vFrame, 0xaaaaaa, 0.3); # Frame
        addPoints(vPunch, 0x3498db, 0.5); # Punch
        addPoints(vStrip, 0xf1c40f, 0.4); # Strip
