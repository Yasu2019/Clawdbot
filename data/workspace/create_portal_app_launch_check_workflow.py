...
WORKFLOW_NAME = "Portal App Launch Check (Daily 07:00 JST)"
ROOT = Path(__file__).resolve().parents[2]
BACKUP_DIR = ROOT / "backups" / "n8n"
STATUS_PATH = ROOT / "data" / "workspace" / "portal_app_launch_check_workflow_status.json"
...
def build_workflow() -> dict:
    return {
        ...
        "nodes": [
            ...
            {
                "id": "node-run",
                "name": "Run Portal Launch Check",
                "type": "n8n-nodes-base.executeCommand",
                "typeVersion": 1,
                "position": [280, 0],
                "parameters": {
                    "command": "python3 /workspace/run_portal_app_launch_check.py",
                    "executeOnce": True,
                },
            },
        ],
        "connections": {
            "Daily 07:00 JST": {
                "main": [[{"node": "Run Portal Launch Check", "type": "main", "index": 0}]]
            }
        },
    }
...
def main() -> None:
    ...
    if existing:
        ...
        status["backup"] = backup_workflow(existing, "portal_app_launch_check_update")
