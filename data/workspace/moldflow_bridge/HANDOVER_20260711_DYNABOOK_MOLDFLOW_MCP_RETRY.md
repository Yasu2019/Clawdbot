...
この時点ではCOM結果だけが未確定だった。最終結果は8.2を参照。
...
## 8.2 2026-07-12 最終COM再試行結果

ASCII版VBSをDynabookへ配布後、MCP経由の32bit probeがPASSした。

```json
{
  "ok": true,
  "exit_code": 0,
  "stdout": "[OK] CreateObject succeeded: synergy.Synergy\nVersion      : 2010\n\n[RESULT] Synergy COM is available for MCP integration.\n",
  "stderr": "",
  "bitness": 32,
...
- COM ProgID: `synergy.Synergy`
- COM bitness: 32bit cscriptで成功
- Moldflow/Synergy version: `2010`
- Study作成・解析・License再起動: なし
- MCP communication + Synergy COM readiness: PASS

配布したVBSのpush済みraw SHA-256:

```text
E8564426541BE44B88A6E8F1EEDB0560DA005C901E344B684F7EAD67E798AE85
```

注意: 当初提示した`D223...`はcommit前作業ツリーの値で誤り。配布ハッシュは必ず
push済み固定commitのraw URLから再取得して計算する。
