...
def patch_engine(text: str) -> str:
    ...
    text = text.replace("             0.0000000000         0.35000E-03", "             0.0000000000         0.50000E-03", 1)
    return text
...
def main() -> None:
    if (ROOT / "data" / "workspace" / "openradioss_pdca_stop.flag").exists():
        raise RuntimeError("stop flag exists; not starting Run43")

    status = {
        "phase": "preparing_run43",
        "container": CONTAINER,
        "run_id": RUN_ID,
        "mode": "bounded_pdca_single_run",
        "threads": THREADS,
        "goal": "confirm clean punch/die material separation, not only normal termination",
        "run42_result": "normal termination but clean cut not confirmed",
        "planned_patch": {
            "FAIL_GENE1_Eps_eff": "0.35 -> 0.22",
            "engine_Tstop_sec": "0.02000 -> 0.02500",
            "ANIM_DT_sec": "0.00035 -> 0.00050 to keep output size bounded",
            "unchanged": ["TYPE25 VISs=0.6", "Inacti=6", "DT/NODA/CST=1.2E-07", "threads=4"],
        },
        "stop_conditions": [
            "run43 reaches normal termination",
            "OpenRadioss terminates on fatal/error condition",
            "user creates data/workspace/openradioss_pdca_stop.flag before next run",
            "no automatic unlimited loop in this script",
        ],
    }
    write_status(status)
    ...
    if "engine_linux64_gf" in top:
        raise RuntimeError("engine is already running; not starting Run43")

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_dir = f"/work/backup_run43_{ts}"
    run(["docker", "exec", CONTAINER, "bash", "-lc", f"mkdir -p {backup_dir} && cp /work/{STARTER} /work/{ENGINE} {backup_dir}/"])

    with tempfile.TemporaryDirectory(prefix="openradioss_run43_") as tmp:
        tmp_path = Path(tmp)
        ...
        starter_local.write_text(patch_starter(starter_local.read_text(encoding="utf-8", errors="replace")), encoding="utf-8")
        engine_local.write_text(patch_engine(engine_local.read_text(encoding="utf-8", errors="replace")), encoding="utf-8")
    ...
    telegram_status = send_telegram("OpenRadioss Run43を開始しました。Run42は切断完了未確認のため、Eps_eff=0.22 / Tstop=0.025 / 4 threadsで再計算します。")
    status.update(
        {
            "phase": "started",
