...
### 🚨 UIファイル変更禁止ルール (P022) — 2026-04-19追加

| ID | 約束内容 | ステータス |
|----|---------|----------|
| P022 | **AIはUIファイルのレイアウト・デザインを自律変更してはならない** | ✅ 有効 |

**背景:** 2026-04-19、Gemini Flash がユーザーの指示なしに `iatf_system/app/views/products/index.html.erb` のレイアウトを大幅変更した。復旧に多大な時間とAPIコストを消費。

**適用ファイル（変更禁止）:**
- `iatf_system/app/views/**/*.html.erb` — Railsビューファイル
- `iatf_system/app/assets/stylesheets/**` — CSSファイル
- `data/workspace/apps/**/*.html` — ポータルUI

**変更が必要な場合の手順:**
1. 変更内容・対象ファイル・変更範囲を必ずPlanとして先に提示
2. ユーザーが明示的に「承認」または「実行」と返答してから作業開始
3. 承認なしの変更は推測であっても絶対に実行しない
