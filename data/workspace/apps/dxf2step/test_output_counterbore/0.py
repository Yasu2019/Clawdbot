...
importDXF.insert("/home/node/clawd/apps/dxf2step/test_output_counterbore/0.cleaned.dxf", "DXFImport")
...
if edges:
    try:
        ...
        if wires:
            ...
            if base_faces:
                ...
                for f in base_faces:
                    try:
                        solids.append(f.extrude(App.Vector(0, 0, 10.0)))
                    except Exception as se:
                ...
                if solids:
                    ...
                    for inner, outer in concentric_pairs:
                        try:
                            ...
                            pocket = Part.makeCylinder(R2, h, App.Vector(cx, cy, 10.0 - h), App.Vector(0, 0, 1))
                    ...
                    result.exportStep("/home/node/clawd/apps/dxf2step/test_output_counterbore/0.step")
                    try:
                        out_doc = App.newDocument("LayerModel")
                        obj = out_doc.addObject("Part::Feature", "LayerSolid")
                        obj.Shape = result
                        out_doc.recompute()
                        out_doc.saveAs("/home/node/clawd/apps/dxf2step/test_output_counterbore/0.FCStd")
                        print(f"Saved FCStd: /home/node/clawd/apps/dxf2step/test_output_counterbore/0.FCStd")
                    except Exception as fce:
                        print(f"FCStd save failed: {fce}")
                    print(f"Exported: /home/node/clawd/apps/dxf2step/test_output_counterbore/0.step  faces={len(result.Faces)}")
                else:
    ...
    print("No edges found in /home/node/clawd/apps/dxf2step/test_output_counterbore/0.cleaned.dxf")
