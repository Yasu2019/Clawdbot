...
views_info = [
    ('front', '/home/node/clawd/apps/dxf2step/jobs/20260228_093606_f2c226a6/output/View.cleaned.dxf'),
    ('top', '/home/node/clawd/apps/dxf2step/jobs/20260228_093606_f2c226a6/output/ProjItem001.cleaned.dxf'),
    ('right', '/home/node/clawd/apps/dxf2step/jobs/20260228_093606_f2c226a6/output/ProjItem.cleaned.dxf'),
]
...
if len(slabs) >= 2:
    try:
        ...
        if vol > 0:
            ...
            # Merge coplanar / same-curvature faces -> clean B-Rep solid
            try:
                cleaned = result.removeSplitter()
                if getattr(cleaned, 'Volume', 0) > 0:
                    result = cleaned
                    print('removeSplitter done - faces after cleanup:', len(result.Faces))
            except Exception as e:
                print('removeSplitter skipped:', e)
            result.exportStep('/home/node/clawd/apps/dxf2step/jobs/20260228_093606_f2c226a6/output/combined.step')
            print('Reconstruction complete - volume:', result.Volume,
                  '- faces:', len(result.Faces))
        else:
            print('Intersection empty, falling back to compound')
            Part.makeCompound(slabs).exportStep('/home/node/clawd/apps/dxf2step/jobs/20260228_093606_f2c226a6/output/combined.step')
    except Exception as e:
        print('Intersection failed, compound fallback:', e)
        Part.makeCompound(slabs).exportStep('/home/node/clawd/apps/dxf2step/jobs/20260228_093606_f2c226a6/output/combined.step')
elif len(slabs) == 1:
    slabs[0].exportStep('/home/node/clawd/apps/dxf2step/jobs/20260228_093606_f2c226a6/output/combined.step')
