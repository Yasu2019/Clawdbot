...
**動画生成が実際に通りました。** これまでffmpegの黒画面5秒しか出力実績が無かった状態から、
Wan2.2 TI2V-5B による実動画（640x384 / 81frames / 24fps / 3.4秒）の生成まで到達しています。

### 追加

- `workflows/wan22/wan22_ti2v_5b_t2v_api.json` — Wan2.2 TI2V-5B の T2V ワークフロー（17トークン）。
  `services/comfyui_fullkit/workflows/wan22_t2v_api.json`（先行実装）をトークン化。
  ノードIDは追跡できるよう先行実装に合わせた。
- `scripts/run_comfy_workflow.py` に `wan22_t2v` プリセットを追加。
- `workflows/wan22/README_JA.md` — トークン表・実測値・GPU調停手順。

### 実測（RTX 5060 Ti 16GB / ComfyUI 0.26.0 GPU版）

| 設定 | 所要 |
|---|---|
| 640x384 / 33frames / 20steps（初回・モデルロード込み） | 220秒 |
| 640x384 / 81frames / 20steps（ウォーム） | **79秒** |

初回220秒のほとんどはモデルロード。事前検証は初回から `OK` で、投入は一発で通った。

### 目視確認の結果

- 81フレーム版: フレーム0/20/40/60/80を抽出。被写体の形状破綻なし、背景が連続的にパン、
  `scene_score` 0.003〜0.034 で不連続なカット・フリッカーなし → **合格**
- 33フレーム版: 画質は良いが**動きがほとんど出ない**。尺は81フレームを既定とする

### GPU調停で判明した点

`scripts/gpu_arbiter.py` のリースが `rl_diag` に保持されたままだった
（2026-08-09取得 / TTL 4時間 → 2026-08-10 11:51 に期限切れ、`lease_live: false`）。
`reap` で回収してから `acquire --owner comfyui --priority 10` を取得し、生成後に `release` した。
本アプリのCLIはリースを自動取得しないため、長時間生成の前に手動で取ること。
