...
STATUS_PATH = ROOT / "data" / "workspace" / "n8n_trigger_workflows_now_status.json"
...
TARGETS = {
    ...
    "7CpooDw45JT71deJ": "Daily Trend Opportunity Report (20:30 JST)",
...
def poll_execution(workflow_id: str, started_after_iso: str, timeout_seconds: int = 240) -> dict | None:
...
def main() -> None:
    ...
    for workflow_id, name in TARGETS.items():
        ...
        backup_path = backup_workflow(wf, "pre_trigger_now")
    ...
    for target in status["targets"]:
        target["execution"] = poll_execution(target["workflowId"], started_after_iso)
        write_status(status)
