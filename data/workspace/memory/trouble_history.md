...
### [自然歩容・最終結論] 足上げ非対称は本質的(自発的対称性破れ) (2026-07-25)

- 5世代の再学習(v1 augmentation / v2-v2b +対称性報酬+強pose_prior / v3 +足上げ直接ペナルティ / v4 +対称参照)+**本体対称化(symbody)**まで投入。
- 結果: hip対称・支持相人間並み(0.67/0.32)・見た目自然は達成。だが**足上げ高さの左右差(片足~7cm/片足~25cm)はどの手法でも消えず**、優位脚は run毎に左右入替。
- **本体を完全対称化(|xL+xR|=0)しても足上げ非対称は残存**→原因は本体/参照の非対称ではなく、**二足歩行リミットサイクルの自発的対称性破れ**(完全対称系でも歩容が片側へ分岐する既知の力学現象)。私の「本体非対称が真因」仮説は誤り(本体非対称は実在の発見でhipは対称化したが、足上げ非対称の主因ではなかった)。
- 教訓: 完全な歩容対称を強制するには超強ペナルティが要り歩容が不自然化するトレードオフ(v4でsurvival悪化を実証)。映像用途では現状の自然歩容で十分。ユーザー選択Aで現状採用。
- 採用ckpt: `known_good/walk_rsl_natural_gait_ADOPTED.pt`(hip対称版)。
