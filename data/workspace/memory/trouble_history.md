...
## [T082] Portalアプリの巨大inline scriptがパース不能だとFACTもレポートも出ない (2026-08-27)

- 症状: CETOL `index.html` が永遠に「cetol_reports.json を読み込んでいます」。JSONは HTTP 200 / 260KB。
- 真因: snap_fit の後に軸受アニメ重複があり `} else if (bolt_joint)` が SyntaxError。FACT fetch が同一script末尾のため一緒に死んだ。
- 対策: 重複ブロック削除 + golden/reports のFACTバー。VIAIは :18010 の `/api/health`+`/api/metrics`。商用Cetol/Moldflow/AOIとは書かない。
- 検証: `node --check` 成功。ブラウザ FACT = golden PASS 2026-08-26 max_err=0.512% / reports 2026-07-15 STALE 30件。
- 再利用: IF アプリ画面が読込のまま AND JSONは200 THEN 巨大scriptを抽出して node --check。FACTは独立IIFEが安全。INC-189 / S033 / bd `app-page-fact-bar-cetol-viai-20260827`。

## [T079aa] 生存率は評価窓依存だった。踏破率で全チェックポイントを測り直した (2026-08-20)

### 訂正

本セッションで報告した survival は**すべて特定の評価窓での値**だった。

    同じ v33 を stairs単体で評価:  15秒窓 0.730 / 30秒窓 0.000
    平地は窓依存しない:            10秒 0.984 / 20秒 0.980 / 30秒 0.965

原因は地形コースが有限長であること。単一地形は全長3.6m程度で約13秒で走り切るが、
その先に地形ジオメトリが無いのに terrain_dz は上限値を返し続ける。

### 踏破率(到達prog / コース全長)で統一評価した結果

| 地形 | 踏破率(平均) | 中央値 | 完走率 |
|---|---|---|---|
| 平地 v26 | 99.3% | 100% | 98.8% |
| 階段昇段 v33 | 94.2% | 100% | 83.6% |
| 階段降段 v35 | 90.6% | 100% | 74.6% |
| 上り斜面 v36 | 98.9% | 100% | 94.9% |
| 下り斜面 v39 | 99.2% | 100% | 97.7% |
| **corridor v42** | **20.6%** | 17.7% | **0.0%** |

条件: push 0.35 + DR、窓はコース長÷指令速度の1.6倍、**参照モーションは学習時と同じ設定**。

### 測定スクリプト側のバグ2件(修正済み)

1. **参照モーションを常に渡していた**。降段/斜面は参照なしで学習したのに階段用の
   参照を与えており、観測が変わって stairs_down が 14.8%(検証済みの travel 3.947m・
   目視確認と矛盾)になっていた。修正後 90.6%。
2. **height_scan を常に有効にしていた**。平地方策は scan 無し(obs189)で学習しており
   shape mismatch で読めなかった。地形に応じて切替。

`eval_course_completion_WIP.py` として stage_a に配置。

### 結論

全単一地形は中央値100%(半数以上が完走)。corridor は完走ゼロで、単一地形に
明確に劣る。同一基準で測れるようになったため、この比較は有効。

### 教訓

**指標には適用範囲がある。** survival はコース長と窓長の組み合わせでしか意味を持たず、
地形をまたいで比較できない。climb_ratio も降り系では分母が0付近になり発散する
(T079x)。指標を使う前に「何に依存するか」を確認すること。


## [T079w] 正規の基準定数を「マジックナンバー」と誤認して削除した (2026-08-18)

### 何をしたか

slope_down が斜面を降りない症状(climb_ratio -0.167)を追う中で、傾斜板の配置式

    zc = -0.92 + sign * half * sin(th)

の `-0.92` を「ハードコードされたマジックナンバー」と断じ、除去する変更をコミットした。
実際には **同一リポジトリ内の2箇所で名前付き定義された正規の基準値**だった:

    train_v50_walk_tracking.py:145  FLOOR_TOP = -0.92   # 床plane天面z(モデル固有)
    v50_walk_env.py:60              FLOOR_Z   = -0.92

この誤りに基づいて、有効だった slope_up の検証済み ckpt (v36) を INVALID_ にリネームし、
誤った前提で再学習(v38)まで起動した。v38 で travel 0.377m/15秒(本来4m)と機体が
助走路から一歩も出ない結果になって初めて誤りに気づいた。すべて復旧済み。

### 4段階の失敗

1. **基準の異なる量を引き算した**。ジオメトリは世界座標、terrain_dz は床面(-0.92)を
   0とする相対高さ。差が一定 -0.920 になるのは当然で、不整合ではなく**座標基準の差**。
2. **定義を検索せずにマジックナンバーと断じた**。grep 1回で FLOOR_TOP に行き当たった。
3. **検証が循環していた**。修正後「差 0.000」を確認したが、それは自分の誤った式が
   自分自身と一致しただけ。物理シミュレータによる独立検証ではなかった。
4. **仮説確定前に破壊的行動を取った**。コミット・有効成果の無効化・長時間ジョブ起動。

### 恒久対策(グローバルルール化)

`python scripts/check_constant.py <値>` を新設。exit 2 なら名前付き定義あり=消すな。
今回の -0.92 で検証し、FLOOR_TOP / FLOOR_Z の2件を正しく検出することを確認した。

### 教訓

**一定オフセットは「バグの証拠」ではなく「基準が違う証拠」であることの方が多い。**
今回の症状(slope_down が降りない)は依然として未解決であり、原因診断はやり直し。
症状が本物でも、原因の仮説が正しいとは限らない。


## [T079u] push強度のカリキュラムで外乱耐性が5.2倍になった (2026-08-17)

### 結果

外乱下 survival が **0.143 -> 0.744**。しかも全条件で同時に改善し、
v32(一定push長時間学習)で起きたトレードオフが無かった。

| 条件 | v30 | v32(6000iter) | **v33(カリキュラム)** |
|---|---|---|---|
| 無擾乱 | 0.852 | 0.635 | **0.896** |
| push 0.35 | 0.488 | 0.418 | **0.639** |
| **push + DR** | 0.143 | 0.227 | **0.744** |
| height_gained | +1.065 | +1.074 | **+1.075 m** |

### 仮説と実装

「いきなり最終強度の外乱を与えると、方策が『外乱に耐える』より先に『歩く』を
壊されて学習が進まない」。`--push-curriculum-frac F` を追加し、push_vel を学習の
最初のF割で 0 -> 設定値まで線形に上げ以後維持する(F=0.3 で検証)。
push_vel は毎ステップ cfg から読まれ学習ループは25iter毎のチャンク実行なので、
チャンク境界で書き換えるだけで実現できた(env再生成不要)。既定0.0で無効。
...
v29/v32 はいずれも 1000iter で頭打ちだったが、v33 だけ伸び続けた。

### ただし追加学習は逆効果だった(v34)

「まだ伸びしろがある」と見て v33 から +3000iter したが**全条件で悪化**した:

    無擾乱     0.896 -> 0.781
    push       0.639 -> 0.598
    push + DR  0.744 -> 0.582

学習曲線も ret 109->111-121 で頭打ち、fall_rate は 0.85 -> 0.96 と悪化していた。
カリキュラムの効果は 3000iter で出尽くしており、その先は回帰。退避保存が効いた。

### 教訓

**外乱を含む学習では、強度のカリキュラムが「長時間学習」より圧倒的に効く。**
v32 は 6000iter かけて 0.227 止まりだったが、v33 は 3000iter で 0.744 に達した。
時間ではなく「与える順序」の問題だった。

また、実験前の私の見立ては2回とも外れている:
 - (B)は「標準的手法レベルの根拠で空振りの可能性が高い」-> 最大の改善だった
 - v33は「まだ伸びしろがある」-> 追加学習は回帰だった
見立てではなく実測で判断すること。


## [T079t] 外乱耐性の実態と、roll解放の失敗 (2026-08-16)

### これまでの survival 報告値は外乱を外した条件だった(訂正)

階段の評価を一貫して push_vel=0.0 / DR無効で行っていた。学習時は
push 0.35 m/s(5秒毎)+ DR(質量・ゲイン ±10%)が入る。同条件で測り直すと:

| 条件 | survival (v30) |
|---|---|
| 無擾乱(これまでの報告値) | 0.852 |
| push 0.35 のみ | 0.488 |
| **push + DR(学習時と同条件)** | **0.143** |

学習ログの fall_rate 0.89-0.93 と整合する。height_gained は全条件で +1.06m を
維持しており「生き残れば必ず完登するが、外乱下では85%が転倒する」状態。
実環境転移を考えるとここが完成度の主要ギャップ。

### roll 解放は逆効果だった(仮説の棄却)

実測: push下で ankle_roll が可動域±14deg の 65% の時間で端に張り付く一方、
hip_roll は ±18deg のうち端が 4.4-15.0% のみで余裕があった。
「股関節で大きく修正し足首で微調整」という本来の二段構えが働かず、足首だけで
耐えて飽和していると解釈した。

原因候補として、参照モーションが roll 4DOF をゼロで埋めており(bvh_retarget の
`_pad_rolls`)、pose_prior がそこへ引き戻している点を疑った。同 docstring は
「rollの使い方(バランス)は参照ではなくRLが発見する」と書いているが、実際には
発見を妨げている、という仮説。

pose_prior を矢状面12DOFのみに限定して検証した結果、**全条件で悪化**:

| 条件 | v30 | v31(roll解放) |
|---|---|---|
| 無擾乱 | 0.852 | 0.652 |
| push 0.35 | 0.488 | 0.322 |
| push + DR | 0.143 | 0.057 |

仮説は棄却。roll をゼロへ引き戻す拘束は、少なくとも現状では**害ではなく助け**
だった(自由度を増やすと探索が発散したか、ゼロ近傍が良い解だった)。
コードは revert 済み。ckpt は latest_ROLLFREE_WORSE_REJECTED.pt として保持。

### 教訓

「可動域に余裕があるのに使われていない」は、必ずしも「拘束が邪魔している」を
意味しない。使われていないこと自体が最適解でありうる。飽和(ankle 65%)は症状
として目立つが、それを解消する方向が性能を上げるとは限らない。
**構造的欠陥(質量・到達不能・符号バグ)と、単なる特性の違いを区別すること。**
前者は直せば大きく効き、後者は触ると悪化する。


## [T079r] 階段昇段 成立 — 真因は報酬ではなく「機体が自重を支えられない」ことだった (2026-08-14)

### 結論

**平均5.32段(最大6段)を登り、10秒間無転倒**。3週間・20回以上の報酬調整で
survival 0.0 を動かせなかった課題が、機体の質量とゲインを直した1回で成立した。

| 指標 | 旧機体(238kg) | **新機体(60kg)** |
|---|---|---|
| 片足が2段目に乗った割合 | 1.2 % | **93.0 %** |
| 両足が2段目に揃った割合 | 0.0 % | **92.2 %** |
| 登った段数(平均) | 0.85 段 | **5.32 段** |
| 生存時間(平均) | 2〜3 s | **11.21 s** |
| 平地の単脚支持率 | 0.61〜0.67 | **0.914〜0.921**(人間基準0.8超) |
| 平地 survival | 0.94 | **1.000** |

### 真因

**平地でゼロ行動(既定姿勢の保持指令のみ)でも1.5秒で胴体zが 0.428 -> 0.055m まで
潰れていた。** 方策以前に、機体が自重を支えられていなかった。

1. **質量が非物理的**: カプセル体積から逆算した密度が upper_leg 8470 / lower_leg 10032
   kg/m3(鉄鋼7850を超え、鉛11340に迫る)。2026-07-10 のトップヘビー対策
   (torso x0.7 / 脚 x1.4)が、元から重い脚をさらに重くしていた。総質量238kgは
   保存される設計だったため総量だけ見ても気づけなかった。
   実在機比較(全長1.40m相当): G1 27.6 / H1 26.1 / ASIMO 41.5 / Atlas 59.3 に対し **170.0 kg-per-m**。

2. **ゲイン不足**: KP/質量が 1.26(G1は4.29)。単脚支持の重力トルク(腕0.10mで233N*m)に
   PDが釣り合うには膝の偏差44.6degが要るが、膝の可動域は40degしかない。
   膝が限界まで折れて支えられない。追従誤差が全区間20〜80degだったのはこのため。

### 対処
MASS_REDIST に _MASS_SCALE=0.2521 を掛け 238.0 -> 60.0 kg(重心を下げる比は保持)。
KP を G1 の KP/質量比 4.29 に合わせ x0.857、KV は減衰比保存で x0.465。

### 教訓(最重要)

**「歩容が不自然」「double supportに偏る」「潰れる」を報酬設計で直そうとしていたが、
相当部分は機体が重すぎたことに起因していた。** 7/25の pose_prior 強化、7/27の
biomechanics ルール5件、8/12-13の各種修正は、いずれも壊れた土台の上の対症療法だった。
新機体では**調整なしで**単脚支持率が人間基準を超えた。

**恒久ルール**: 学習が進まないとき、報酬を触る前に
  1. ゼロ行動で自重を支えられるか(静的保持テスト)
  2. 質量/寸法が実在機と比べて妥当か(kg-per-m 等の無次元量で比較)
  3. KP/質量が実績のある機体と同オーダーか
を先に確認する。報酬は「支えられる機体」を前提にした調整手段でしかない。

VERIFIED: known_good/walk_rsl_flat_v26_20260814_60kg_surv1.00_1foot0.92.pt
          known_good/walk_rsl_stairs_v27_20260814_CLIMB5.3steps_surv0.54.pt


## [T079n] 階段用の報酬を延々調整していたが、ロボットは階段に一度も到達していなかった (2026-08-13)

**発端**: 遊脚クリアランス不足を疑って実測したところ中央値 0.1137m と段高 0.10m を既に超えており、
仮説が外れた。より根本的に「そもそも階段まで届いているのか」を地形形状から確認した。

**判明した事実**:
```
最初の段の立ち上がり : 前進 0.61 m 地点
実測エピソード長     : 3.09 秒
8秒評価窓での前進    : 0.156 m
指令速度 cmd_vx      : 0.05〜0.08 m/s  (私が設定)
```
cmd_vx 0.059 では 0.61m の段に届くまで **10秒以上**かかる。エピソードは3秒で終わる。
**ロボットは平地の助走区間で転んでいただけで、階段に一度も触れていなかった。**

つまり climb_progress / climb_stagnation / 段差カリキュラム(段高0.05m化)など、
**階段用に調整してきた報酬は一度も発火していなかった**。無意味な調整を繰り返していた。

**なぜこうなったか**: CMU参照モーションの `clip_vx_mps = 0.0585` に合わせて cmd_vx を下げたため。
しかし参照の clip_vx は「その人間クリップがたまたま何m/sだったか」でしかなく、
**ロボットに命令すべき速度ではない**。2026-08-10 の平地成功例は cmd_vx 0.25〜0.33 で travel 2.4m。

**恒久ルール**:
1. 地形タスクを設定したら、まず **「目標地物までの距離 ÷ 指令速度 < エピソード長」** を確認する。
   満たさなければ、その地形用の報酬は一度も発火しない。
2. 参照モーションの clip_vx をそのまま cmd_vx にしない。参照は関節角度の手本であって速度指令ではない。
3. 報酬項を疑う前に **タスクが物理的に到達可能かを先に確認する**。
   本件は係数調整を10回以上繰り返した後に、地形形状を1回測るだけで判明した。


## [T079m] 「生存に価値が無い」報酬設計で16回失敗 / 目視の異常を実体の破綻と即断した誤診 (2026-08-12〜13)

### 突破: alive報酬の欠落が真因だった

階段昇段が16回以上 `survival_rate = 0.0` から動かなかった。真因は**生存に対する報酬が存在しなかった**こと。

`unitree_rl_gym` の G1(公開されている動作実績のあるヒューマノイド設定)を `D:\RobotAI\repos\` に取得し、
reward_scales を機械的に突き合わせて判明した:

```
alive        当方: (項自体が無い)     G1: +0.15/step
termination  当方: -100.0  → 実効 -2.0(一発のみ。全係数に dt を掛ける規約のため)
毎ステップの罰: -0.05 〜 -0.24
```

50ステップ生き延びると -2.5〜-12 を払うのに、転ぶコストは -2.0 だけ。**数値上、転んだ方が得**だった。
`alive = 7.5`(dt規約換算で実効 0.15/step、G1と一致することを実測確認)を追加した結果:

| 指標 | 追加前 | 追加後 |
|---|---|---|
| survival_rate (fresh) | 0.0 | **0.082** |
| first_fall_sec | 0.96s | **3.86s** |
| mean_episode_len | 1.06s | **2.70s** |
| mean_reward_per_step | -0.111 | **+0.028**(初のプラス) |
| vx_mean | -0.114(後退) | **+0.034**(前進) |

CLAUDE.md のグローバルルール「ローカルに知見が無ければ外部を調べる/公開実装の設定ファイルを先に読む」が
またも正しかった。数時間の推測より、実績ある設定ファイル1枚の方が情報量が多い。

### 誤診: レンダリング上の「腕分離」を物理破綻と即断した

追加学習(2000→5000iter)後のフレームで両腕が胴体から分離して見え、**物理破綻と判定して
チェックポイントを却下し、対策として `dof_pos_limits` まで追加した。これは誤りだった。**

リンク間距離を実測したところ:
```
torso - upper_arm_L : 0.5630 m  ← 8秒間、小数点4桁まで完全に一定(剛体連結)
肩関節角度          : -26〜+24°  ← 可動域 ±100° の 1/4 にも達していない
```

発散していれば距離が伸びるが、一定だった。**見た目の隙間(胴体カプセルと腕リンクの形状特性)を
実体の破綻と読み違えた。** `dof_pos_limits` は誤った前提で入れた項(ただしG1にもある標準項で、
実測上ほぼ発火しないため害は無い)。

さらに、却下の根拠として引用した `height_gained_m = +0.022`(初のプラス)も**測定ノイズ**だった。
同一チェックポイントの再測定で -0.033。3つのチェックポイントを公平比較すると全て負で、
**どれも実際には登っていない**。

**教訓**: 目視確認は必須だが、「見た目の異常」は**実体の破綻とイコールではない**。
プロジェクトには既に「表示の化けと実ファイルの破損を混同しない」という同種の教訓があり
(文字化け防止ルール)、それを3Dレンダリングにも適用すべきだった。
異常に見えたら、**却下や対策の前に数値で裏を取る**(リンク間距離・関節角度)。
また単発の verify 値で判断せず、複数回または複数チェックポイントの比較で確かめる。

### 現在地
survival 0.082 / travel +0.156 / height_gained -0.078。
「段差の手前で3秒前後、自然な姿勢で立って軽く前進できる」段階。**昇段は未達成。**
最良: `known_good/walk_rsl_stairs_v19_20260812_surv0.078_FIRST_NONZERO.pt`
