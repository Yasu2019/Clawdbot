...
- Verification: PowerShell AST parse PASS; 32 focused launcher/preflight/checkpoint tests PASS. Includes actual cross-process named-mutex exclusion, Bash process-family audit, mocked worker finalization, atomic create/replace with failure preservation, worker rejection of mismatched manifest ownership, and Git Bash runtime/order checks that refuse existing preflight start/result markers before task/solver dispatch without altering evidence. Worker accepts validated scalar config, reconstructs fixed shell template, and records solver status facts; it has side-effect-free `-ValidateOnly`.
