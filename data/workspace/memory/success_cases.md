...
- Verification: PowerShell AST parse PASS; 30 focused launcher/preflight/checkpoint tests PASS. Includes actual cross-process named-mutex exclusion, Bash process-family audit, mocked worker finalization, atomic create/replace with failure preservation, and worker rejection of a manifest whose schema/run identity (task, unit, case path) does not match. Rejected foreign JSON remains unchanged. Worker accepts validated scalar config, reconstructs fixed shell template, and records solver status facts; it has side-effect-free `-ValidateOnly`.
