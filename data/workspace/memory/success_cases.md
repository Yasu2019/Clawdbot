...
- Verification: PowerShell AST parse PASS; 30 focused launcher/preflight/checkpoint tests PASS. Includes actual cross-process named-mutex exclusion, Bash process-family audit, mocked worker finalization, and a Windows PowerShell integration test that atomically creates/replaces the host manifest, proves a replacement failure preserves the previous JSON, and verifies no temporary/backup residue. Worker accepts validated scalar config, reconstructs fixed shell template, and records solver status facts; it has side-effect-free `-ValidateOnly`.
