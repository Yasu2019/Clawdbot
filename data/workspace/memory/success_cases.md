...
## 2026-08-10 INC-188 external-solid contact and natural-step reference

1. **Problem:** Initial penetration and catastrophic added mass appeared at cut.
2. **Root cause:** Coincident SH3N skins duplicated solid boundaries; continuous
   NODA/CST added unbounded mass following element deletion.
3. **Fix:** Use `/SURF/PART/EXT`, remove skins, and compare `/DT/NODA/0` truth.
4. **Verification:** Trial M Starter 0 errors/warnings; Trial Q ERR=0%, DM/M=0
...
- Verification: PowerShell AST parse PASS; 34 focused launcher/preflight/checkpoint tests PASS. Includes actual cross-process named-mutex exclusion, Bash process-family audit, mocked worker finalization, atomic create/replace with failure preservation, worker rejection of mismatched manifest ownership, fresh-case marker refusal before side effects, terminal case/time consistency, checkpoint-field/fatal/exit success contract, ambiguous dispatch resolution, refusal to overwrite an existing host manifest, and serialization of worker manifest read/modify/write under the launch mutex. Default host manifests now use unique per-run names. A dispatch timeout is not classified as a definite reject; monitor waits up to 90s for unit activation. Worker accepts validated scalar config, reconstructs fixed shell template, and records solver status facts; it has side-effect-free `-ValidateOnly`.
...
## [S039] CalculiX/Elmer shrinkage wiring proved with independent analytical smokes (2026-09-20, Codex)

- Problem: solidification eigenstrain was previously written only as audit CSV/comment, and Elmer existed as an input skeleton; neither path proved that a negative shrinkage history produced the correct solved displacement.
- Diagnosis: inspected generated decks and official CalculiX initial-strain semantics, then used one tetrahedron with minimum rigid-body constraints so sign and magnitude have closed-form answers. The first Elmer verification false-HOLD was traced to results under `mesh/`, not a solver failure.
- Solution: write CalculiX `*INITIAL CONDITIONS, TYPE=PLASTIC STRAIN` plus per-step `*INITIAL STRAIN INCREASE`; add real Docker CalculiX and Elmer smokes, recursive Elmer artifact discovery, SHA provenance, and fail-closed analytical comparisons.
- Evidence: CalculiX 2.16 produced DAT/FRD/STA; `E=-0.01` gave edge `0.0098994949 m` versus exact `0.0098994949366 m`. Elmer 8.4 with virtual `alpha=1e-4/K`, `Delta T=-10 K` gave all axial displacements `-1e-5 m`, relative L2 `0`. Targeted tests passed.
- Reuse: IF a solver coupling is claimed THEN run a minimum constrained specimen with an analytical sign/magnitude oracle and retain native result artifacts. IF Elmer uses `Mesh DB` subdirectories THEN discover outputs recursively and bind relative path plus SHA; never infer failure from a root-only search.
