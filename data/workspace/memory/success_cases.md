...
- Verification: PowerShell AST parse PASS; 29 focused launcher/preflight/checkpoint tests PASS, including actual cross-process named-mutex exclusion plus Bash process-family and mocked worker finalization. Worker accepts validated scalar config, reconstructs fixed shell template, and records solver status facts; it has side-effect-free `-ValidateOnly`.
