...
- Verification: PowerShell AST parse PASS; 33 focused launcher/preflight/checkpoint tests PASS. Includes actual cross-process named-mutex exclusion, Bash process-family audit, mocked worker finalization, atomic create/replace with failure preservation, worker rejection of mismatched manifest ownership, fresh-case marker refusal before side effects, terminal case/time consistency, checkpoint-field/fatal/exit success contract, ambiguous dispatch resolution, and refusal to overwrite an existing host manifest. Default host manifests now use unique per-run names. A dispatch timeout is not classified as a definite reject; monitor waits up to 90s for unit activation. Worker accepts validated scalar config, reconstructs fixed shell template, and records solver status facts; it has side-effect-free `-ValidateOnly`.
