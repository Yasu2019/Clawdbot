...
- Verification: PowerShell AST parse PASS; 32 focused launcher/preflight/checkpoint tests PASS. Includes actual cross-process named-mutex exclusion, Bash process-family audit, mocked worker finalization, atomic create/replace with failure preservation, worker rejection of mismatched manifest ownership, fresh-case marker refusal before side effects, and terminal-result checks for exact case identity, finite times, successful exit code, and reaching requested end time before reporting completion. Worker accepts validated scalar config, reconstructs fixed shell template, and records solver status facts; it has side-effect-free `-ValidateOnly`.
