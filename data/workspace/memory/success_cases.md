...
- Verification: PowerShell AST parse PASS; 31 focused launcher/preflight/checkpoint tests PASS. Includes actual cross-process named-mutex exclusion, Bash process-family audit, mocked worker finalization, atomic create/replace with failure preservation, worker rejection of mismatched manifest ownership, and a Git Bash runtime check that refuses existing preflight start/result markers without altering them. Worker accepts validated scalar config, reconstructs fixed shell template, and records solver status facts; it has side-effect-free `-ValidateOnly`.
