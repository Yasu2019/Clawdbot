...
- Verification: PowerShell AST parse PASS; 22 focused launcher/preflight/checkpoint tests PASS; `-ValidateOnly` valid case and NaN rejection exercised in Windows PowerShell with no WSL/task/solver calls. Keepalive worker records final status, exit code, and bounded output into the host manifest.
