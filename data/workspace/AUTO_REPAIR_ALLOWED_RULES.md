...
## 2026-08-01 実行基盤の恒久対策

- OSファイルロックで同時実行を1件に制限する。
- 子コマンドのタイムアウト時は、親だけでなくプロセスツリーを終了する。
- status JSONは一時ファイルから原子的に置換する。
- 未処理例外は `step=failed` と `requiresEscalation=true` を記録する。
- タイムアウトは修復成功として扱わず、既存の試行回数ゲートに従う。
